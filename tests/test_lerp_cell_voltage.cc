...
void test_lerp_cell_voltage_source_voltage_buf(
  ...
  if (buf.the_error != NONE) {
    ...
    BOOST_REQUIRE_EQUAL(
        lerp_cell_voltage_linear(linear_buf_pointer,
                                 buf.charge_discharged_from_cell),
        buf.cell_voltage_source_voltage);
    BOOST_REQUIRE_EQUAL(error, buf.the_error);
    error = NONE;
    BOOST_REQUIRE(*linear_buf_pointer == linear_buf);
    const LerpCellVoltageBinaryBufType binary_buf = *binary_buf_pointer;
    BOOST_REQUIRE_EQUAL(
        lerp_cell_voltage_binary(binary_buf_pointer,
                                 buf.charge_discharged_from_cell),
        buf.cell_voltage_source_voltage);
    BOOST_REQUIRE_EQUAL(error, buf.the_error);
    error = NONE;
